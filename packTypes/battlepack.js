module.exports = {
    ...
    openPack(msg, args, con, pack, Discord, numToPull) {
        ...
        for (let p = 0; p < numToPull; p++) {
            ...
            pagesR[p] = strr;
            if (args[1].toLowerCase() === "bp01") {
                let sRare = getRandomCards(pack.get(args[1].toLowerCase()).cards, pack.get(args[1].toLowerCase()).starnum);
                for (let i = 0; i < sRare.length; i++) {
                    if (newCard.includes(sRare[i])) {
                        let curIndex = newCard.indexOf(sRare[i]);
                        if (newRarity[curIndex] === "Starfoil") {
                            numNew[curIndex] += 1;
                        } else {
                            newCard[newIndex] = sRare[i];
                            newRarity[newIndex] = "Starfoil";
                            numNew[newIndex] = 1;
                            newIndex++;
                        }
                    } else {
                ...
                pagesSr[p] = strsr;
            } else if (args[1].toLowerCase() === "bp02") {
                let sRare = getRandomCards(pack.get(args[1].toLowerCase()).cards, pack.get(args[1].toLowerCase()).starnum);
                for (let i = 0; i < sRare.length; i++) {
                    if (newCard.includes(sRare[i])) {
                        let curIndex = newCard.indexOf(sRare[i]);
                        if (newRarity[curIndex] === "Mosaic") {
                            numNew[curIndex] += 1;
                        } else {
                            newCard[newIndex] = sRare[i];
                            newRarity[newIndex] = "Mosaic";
                            numNew[newIndex] = 1;
                            newIndex++;
                        }
                    } else {
            ...
            if (rows.length > 0) {
                if (rows[0].points >= pack.get(args[1].toLowerCase()).price * numToPull) {
                    ...
                    if (pagesSr[page] != undefined) {
                        embed.addField("Starfoil", pagesSr[page]);
                    } else if (pagesM[page] != undefined) {
                        embed.addField("Mosaic", pagesM[page]);
                    }
    ...
    viewContents(msg, args, pack, Discord) {
        ...
        let common3 = "";
        let mosaic = "";
        for (let i = 0; i < comList.length / 3; i++) {
            common += comList[i] + "\n";
        }
        for (let i = comList.length / 3; i < comList.length / 3 * 2; i++) {
            common2 += comList[i] + "\n";
        }
        for (let i = comList.length / 3 * 2; i < comList.length; i++) {
            common3 += comList[i] + "\n";
        }
        for (let i = 0; i < rareList.length; i++) {
            rare += rareList[i] + "\n";
        }
        if (name.toLowerCase() === "bpo2") {
            pages = 5;
            mosaic = "Obelisk the Tormentor\nThe Winged Dragon of Ra\nSlifer the Sky Dragon";
        }
                ...
                back.on('collect', async r => {
                    ...
                    switch (page) {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            packEmbed.addField("Mosaic", mosaic);
                            break;
                    }
                    packEmbed.setFooter(`Page ${page} out of ${pages}`);
                    msg.edit(packEmbed);
