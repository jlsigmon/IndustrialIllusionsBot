module.exports = {
    ...
    openPack(msg, args, con, pack, Discord, numToPull) {
        ...
        for (let p = 0; p < numToPull; p++) {
            ...
            if (upChance <= 929) {
                ...
                let secret;
                if (flip === 1) {
                    secret = getRandomCards(pack.get(args[1].toLowerCase()).ultimate, pack.get(args[1].toLowerCase()).secretnum);
    ...
    missingCards(msg, args, con, Discord, pack){
        ...
        let commonList = pack.get(args[1].toLowerCase()).common;
        ...
        con.query(`SELECT * FROM collection WHERE userId = '${msg.author.id}' AND serverId = "${msg.guild.id}" ORDER BY cardName`, (err, rows) => {
            ...
            if (rows.length > 0) {
                ...
                for(var z = 0; z < rows.length; z++){
                    if(commonList.includes(rows[z].cardName) && rows[z].cardRarity === 'Common'){
                ...
                for(var x = 0; x < commonList.length; x++){
                    if(!commons.includes(commonList[x])){
                        card[curPage] += commonList[x] + "\n";
