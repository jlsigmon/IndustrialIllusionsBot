module.exports = {
    ...
    openPack(msg, args, con, pack, Discord, numToPull) {
        ...
        for (let p = 0; p < numToPull; p++) {
            ...
            if (upChance <= 929) {
                ...
                let secret = getRandomCards(pack.get(args[1].toLowerCase()).ultimate, pack.get(args[1].toLowerCase()).ultimatenum);
    ...
    viewContents(msg, args, pack, Discord) {
        ...
        let secretList = pack.get(name.toLowerCase()).ultimate;
        let common = "";
        let common2 = "";
        let rare = "";
        let sRare = "";
                ...
                back.on('collect', async r => {
                    ...
                    switch (page) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            packEmbed.addField("Ultimates", secret);
                            break;
                    }
                    packEmbed.setFooter(`Page ${page} out of ${pages}`);
                    msg.edit(packEmbed);
    ...
    missingCards(msg, args, con, Discord, pack) {
        ...
        let secretList = pack.get(args[1].toLowerCase()).ultimate;
        let commons = [];
        let supers = [];
        let rares = [];
        let ultras = [];
        ...
        con.query(`SELECT * FROM collection WHERE userId = '${msg.author.id}' AND serverId = "${msg.guild.id}" ORDER BY cardName`, (err, rows) => {
            ...
            if (rows.length > 0) {
                ...
                for (var z = 0; z < rows.length; z++) {
                    ...
                    if (secretList.includes(rows[z].cardName) && rows[z].cardRarity === 'Ultimate') {
