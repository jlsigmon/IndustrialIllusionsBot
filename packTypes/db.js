module.exports = {
    ...
    openPack(msg, args, con, pack, Discord, numToPull) {
        ...
        for (let p = 0; p < numToPull; p++) {
            ...
            let upChance = Math.floor(Math.random() * 24);
            let strc = '';
            let strsr = '';
            let strr = '';
            let stru = '';
            ...
            } else if (upChance > 17 && upChance <= 22) {
    ...
    viewContents(msg, args, pack, Discord) {
        ...
        let sRare = "";
        let ultra = "";
        for (let i = 0; i < Math.floor(comList.length / 4); i++) {
            common += comList[i] + "\n";
        }
        for (let i = Math.floor(comList.length / 4) + 1; i < Math.floor(comList.length / 2); i++) {
            common2 += comList[i] + "\n";
        }
        for (let i = Math.floor(comList.length / 2) + 1; i < Math.floor(comList.length / 4) * 3; i++) {
            common3 += comList[i] + "\n";
        }
        for (let i = Math.floor(comList.length / 4) * 3 + 1; i < comList.length; i++) {
            common4 += comList[i] + "\n";
        }
                ...
                back.on('collect', async r => {
                    ...
                    switch (page) {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            packEmbed.addField("Commons", common4);
                            break;
                        case 5:
