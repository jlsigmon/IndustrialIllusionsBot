module.exports = {
    ...
    execute(msg, args, con, Discord){
        ...
        if(args.length === 1){
            ...
            } else if(args[1].toLowerCase() === "rarity:"){
                var rarity = args[2];
                
                    con.query(`SELECT * FROM collection WHERE userId = '${msg.author.id}' AND serverId = "${msg.guild.id}" AND cardRarity = "${rarity}" ORDER BY cardName ASC`, (err, rows) => {
                        if (err) throw err;
                        ...
                        if (rows.length > 0) {
                            ...
                            msg.reply("You currently don't have any cards in that rarity!");
                        }
                    })
                            ...
                            msg.react('◀').then(async r => {
                                ...
                                ford.on('collect',async r => {
                                    ...
                                    if (card[page - 1] !== undefined && card[page - 1] != "") {
                                        ...
                                        for (var c = per; c < per + 25 && c < rows.length; c++) {
                                            if(rows[c].cardName.toLowerCase().includes(cardName.toLowerCase())){
                                                card[page - 1] += rows[c].cardName + "\n";
                                                rarity[page - 1] += rows[c].cardRarity + "\n";
                                                num[page - 1] += 'x' + rows[c].cardNum + "\n";
                                            } else {
